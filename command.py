...
def deposit(bot, update):
	...
	if user is None:
		...
		address = "/home/mobileapi/HTMLCOIN/src/htmlcoind"
...
def tip(bot,update):
	...
	if user is None:
		...
		if target == machine:
			...
			target = target[1:]
			user = update.message.from_user.username 
			core = "/home/mobileapi/HTMLCOIN/src/htmlcoind"
...
def price(bot,update):
	...
	quote_page = requests.get('https://bleutrade.com/api/v2/public/getticker?market=HTML_BTC')
	soup = BeautifulSoup(quote_page.content, 'html.parser').text
	btc = soup[80:]
	sats = btc[:-2]
	bot.send_message(chat_id=update.message.chat_id, text="Htmlcoin is valued at {0} Δ {1} ≈ {2}".format(price,percent,sats) + " ฿")
...
def withdraw(bot,update):
	...
	if user is None:
		...
		if balance < amount:
			...
			bot.send_message(chat_id=update.message.chat_id, text="@{0} has successfully withdrew to address: {1} of {2} HTML" .format(user,address,amount))
...
def marketcap(bot,update):
	...
	name = name_box.text.replace("\n","")
	mc = re.sub(r'\n\s*\n', r'\n\n', name.strip(), flags=re.M)
	bot.send_message(chat_id=update.message.chat_id, text="The current market cap of Htmlcoin is valued at {0}".format(mc))
